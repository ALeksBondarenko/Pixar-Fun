        ...
        addTask { @MainActor in
            do {
                if case .success(let movie, let images, let videos, let casts, let favorite, let watchLater) = self.state {
                    ...
                    if isFvorite {
                        isFvorite = try await !self.repository.removeToFavorites(accountId: Enviroment.accountId, movieId: self.movie.id)
                    } else {
                        isFvorite = try await self.repository.addToFavorites(accountId: Enviroment.accountId, movieId: self.movie.id)
                    }
        ...
        addTask { @MainActor in
            do {
                if case .success(let movie, let images, let videos, let casts, let favorite, let watchLater) = self.state {
                    ...
                    if isWatchLater {
                        isWatchLater = try await !self.repository.removeToWatchLater(accountId: Enviroment.accountId, movieId: self.movie.id)
                    } else {
                        isWatchLater = try await self.repository.addToWatchLater(accountId: Enviroment.accountId, movieId: self.movie.id)
                    }
